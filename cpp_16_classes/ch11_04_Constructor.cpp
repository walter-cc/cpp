...
Student::Student()		//constructor 設定資料成員的初始值
{
    StuID = 920101;
    Score_E = 60;
    Score_M = 80;
...
Student::Student(int id, float E, float M)		//constructor 使用參數設定初始值
{
    StuID = id;
    Score_E = E;
    Score_M = M;
...
int main(void) {
    ...
    Student stud;		//宣告 class Student 的物件，此時會呼叫無參數的建構

    stud.show_data();

    Student stud1(920102, 30, 40);	//宣告 class Student 的物件，此時會呼叫三個參數的建構，此為多載(overload)
